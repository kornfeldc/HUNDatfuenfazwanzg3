<script lang="ts" module>
	...
	export const sheetVariants = tv({
		base: "bg-sidebar backdrop-blur-xl data-[state=open]:animate-in data-[state=closed]:animate-out fixed z-50 flex flex-col gap-4 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
		variants: {
			side: {
...
<script lang="ts">
	...
	let escPressed = (eve: KeyboardEvent) => {
		if (eve.key === 'Escape') {
			eve.stopPropagation();
